...
| `patchState(patch)` | `(patch: StatePatch<State>) => void` | Shallow-merges a partial object into the current state. Accepts a partial object or a function that receives the previous state and returns a partial object. |
...
/** A partial representation of the state used by patchState. Accepts a partial object or a function that returns one. */
