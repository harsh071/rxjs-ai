...
chat.send('Summarize this document');
...
const gpt$ = gptModel.stream(messages);
const claude$ = claudeModel.stream(messages);

race(gpt$, claude$).subscribe((token) => {
  console.log(token);
});
```
...
rxjs-ai includes `createStore` for state and `createViewModel` for derived view state. No external state library needed:
...
// Derive a view model with fine-grained selectors
...
vm.state$.subscribe(console.log);
...
For React, rxjs-ai ships a thin adapter:
...
Vue, Svelte, Angular, Solid — any framework that can subscribe to an Observable (all of them) works with rxjs-ai out of the box.

## Next steps

<CardGrid>
  <LinkCard title="Quick Start" href="/getting-started/quick-start/" description="Build your first streaming chat in 5 minutes." />
  <LinkCard title="createStore" href="/core/create-store/" description="State management primitive." />
