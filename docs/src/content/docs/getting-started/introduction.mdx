...
- **Common patterns** like reactive state and event routing require manual `BehaviorSubject`/`Subject` wiring every time.
...
| Primitive | Purpose |
|---|---|
| `createStore` | Reactive state container with Observable selectors |
...
| **Reactive primitives** | Built-in (`createStore`, `createCommandBus`, `createViewModel`) | Bring your own |
| **Framework support** | Framework-agnostic | React-first |
