...
const chat = createChatController(
  raceAdapters(openaiAdapter, anthropicAdapter),
);
...
```ts
import { Subject, debounceTime, switchMap, scan, catchError, of } from "rxjs";
import { type ChatModelAdapter } from "rxjs-ai";
...
const results$ = search$.pipe(
  ...
  switchMap((query) => {
    ...
    return adapter.complete({ messages, signal: ac.signal }).pipe(
...
> **Note:** The `scan` operator is imported from `rxjs` and used to accumulate streamed chunks into a single string.
...
const chat = createChatController(composedAdapter);
