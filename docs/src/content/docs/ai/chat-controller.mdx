...
const chat = createChatController(myAdapter);
```
...
The `complete` method receives the full conversation history and an `AbortSignal` for cancellation, and must return an `Observable` that emits one or more **chunks** as the model responds.
...
const adapter: ChatModelAdapter = {
  complete({ messages, signal }) {
    ...
    return myLlmStream(messages, signal);
  },
};
...
|---|---|---|
| `model` | `ChatModelAdapter` | **Required.** The model adapter to use for completions. Passed as the first positional argument. |
| `options` | `CreateChatControllerOptions` | Optional configuration object (see below). |
...
| `error$` | `Observable<unknown \| null>` | The latest error, or `null` when cleared. |
